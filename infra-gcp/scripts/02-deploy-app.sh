...
build_and_push_docker() {
    if [[ "$SKIP_BUILD" == true ]]; then
        ...
        IMAGE_TAG=$(gcloud artifacts docker images list \
            "$REGION-docker.pkg.dev/$PROJECT_ID/biocom-api/biocom-mq" \
...
deploy_kubernetes() {
    ...
    if [[ -f "$CONFIGMAP_FILE" ]]; then
        ...
        log_warning "환경별 ConfigMap 파일을 찾을 수 없습니다: $CONFIGMAP_FILE"
...
# 메인 실행
main() {
    log_info "🚀 BIOCOM-MQ Worker 배포 시작! (환경: $DEPLOY_ENV)"
