-- 미션 마스터 데이터 INSERT (V2)
-- 생성일: 2025-10-15
-- 변경사항: code 컬럼 제거, recordType이 유일한 식별자
...
INSERT INTO missions (
  record_type, name, description, points, require_upload, daily_limit,
  specific_day, total_days, category, type, sort_order, is_active
) VALUES (
  'DIET',
  '식단 기록',
...
INSERT INTO missions (
  record_type, name, description, points, require_upload, daily_limit,
  specific_day, total_days, category, type, upload_type, sort_order, is_active
) VALUES (
  'ROUTINE',
  '아랑 피부 루틴',
...
-- 기록형 미션 확인
SELECT record_type, name, type
FROM missions
WHERE type = 'RECORD'
ORDER BY sort_order;

-- 인증샷 필수 미션 확인
SELECT record_type, name, require_upload, upload_type
FROM missions
