 ...
 * 생년월일 기반 유저 삭제 스크립트
 *
 * 사용법: npx ts-node prisma/operation/delete-user-by-birth.ts [birthDate]
 * 예시: npx ts-node prisma/operation/delete-user-by-birth.ts 19900910
 */
...
async function deleteUserByBirth() {
  const birthDate = process.argv[2];

  if (!birthDate) {
    console.error('❌ 사용법: npx ts-node prisma/operation/delete-user-by-birth.ts [birthDate]');
    console.error('   예시: npx ts-node prisma/operation/delete-user-by-birth.ts 19900910');
    process.exit(1);
  }

  console.log(`\n🔍 birthDate ${birthDate} 유저 검색 중...\n`);

  try {
    // 유저 검색
    const user = await prisma.user.findFirst({
      where: { birthDate },
      select: { id: true, name: true, mobile: true, birthDate: true, createdAt: true }
    });

    if (!user) {
      console.log(`❌ birthDate ${birthDate}인 유저가 없습니다.\n`);
      await prisma.$disconnect();
      return;
    }

    const userId = user.id;
    console.log(`✅ 유저 발견: id=${userId}, birthDate=${user.birthDate}, createdAt=${user.createdAt}`);
    ...
    console.log(`\n✅ birthDate ${birthDate} (user_id ${userId}) 관련 모든 데이터 삭제 완료!\n`);
