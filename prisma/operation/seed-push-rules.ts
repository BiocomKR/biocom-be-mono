...
/**
 * CSV에서 PushRule 데이터를 파싱하여 DB에 삽입하는 스크립트
 *
 * 실행 방법:
 * npx ts-node prisma/operation/seed-push-rules.ts
 *
 * 주의: push_rules 테이블이 먼저 생성되어 있어야 합니다.
 * prisma/migrations/push_rule_schema.sql 실행 필요
 */
...
function parseConditionType(row: CsvRow): { type: ConditionType; params: Record<string, any> } {
  ...
  // 장바구니
  if (조건.includes('장바구니') || 랜딩_위치?.includes('장바구니')) {
 ...
 */
function parseLandingType(landingText: string, row: CsvRow): { type: LandingType; params: Record<string, any> | null } {
...
/**
 * 발송 시점을 SendTime으로 변환
 */
function parseSendTime(sendTimeText: string): string {
  ...
  if (text.includes('이탈 후 최초') && text.includes('18시')) {
    return 'ON_FIRST_18_AFTER_EXIT';
  }

  if (text.includes('24시간 경과')) {
    return 'ON_24H_ELAPSED';
  }

  if (text.includes('48시간 경과')) {
    return 'ON_48H_ELAPSED';
  }
...
async function main() {
  console.log('🚀 PushRule 시드 스크립트 시작...\n');
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      const sendTime = parseSendTime(row.발송_시점);
      ...
      for (const persona of personas) {
        ...
        if (msg.title !== maeveMsg.title || msg.body !== maeveMsg.body) {
          messages[persona.key] = msg;
        }
      }

      // senderType 결정 (비고에 "바이오컴 이름으로" 있으면 BIOCOM)
      const senderType: 'BIOCOM' | 'PERSONA' = row.비고?.includes('바이오컴 이름으로')
        ? 'BIOCOM'
        : 'PERSONA';

      const pushCode = generatePushCode(row, i + 1);
      const name = `${row.상황 || '기타'} - ${row.조건 || row.세부_조건_1 || '일반'}`.substring(0, 100);
  ...
  console.log(`✅ 변환된 PushRule 수: ${pushRules.length}\n`);

  // 샘플 출력
  console.log('📋 샘플 데이터 (처음 3개):');
  for (let i = 0; i < Math.min(3, pushRules.length); i++) {
    console.log(`\n--- ${i + 1}. ${pushRules[i].pushCode} ---`);
    console.log(JSON.stringify(pushRules[i], null, 2));
  }
  ...
  if (dryRun) {
    ...
    console.log('예: npx ts-node prisma/operation/seed-push-rules.ts --execute\n');

    // JSON 파일로 출력
    const outputPath = path.resolve(__dirname, 'push-rules-seed-data.json');
    fs.writeFileSync(outputPath, JSON.stringify(pushRules, null, 2), 'utf-8');
    console.log(`📁 JSON 데이터 저장됨: ${outputPath}`);
  } else {
    console.log('\n🔄 DB에 PushRule 삽입 중...');

    // 기존 데이터 삭제 (선택적)
    // await prisma.$executeRaw`TRUNCATE TABLE push_rules RESTART IDENTITY`;
      ...
      try {
        ...
        await prisma.$executeRaw`
          INSERT INTO push_rules (
            push_code, name, description, condition_type, condition_params,
            landing_type, landing_params, messages, send_time, sender_type, is_active
          ) VALUES (
            ${rule.pushCode},
            ${rule.name},
            ${rule.description},
            ${rule.conditionType},
            ${JSON.stringify(rule.conditionParams)}::jsonb,
            ${rule.landingType},
            ${rule.landingParams ? JSON.stringify(rule.landingParams) : null}::jsonb,
            ${JSON.stringify(rule.messages)}::jsonb,
            ${rule.sendTime},
            ${rule.senderType},
            ${rule.isActive}
          )
          ON CONFLICT (push_code) DO UPDATE SET
            name = EXCLUDED.name,
            description = EXCLUDED.description,
            condition_type = EXCLUDED.condition_type,
            condition_params = EXCLUDED.condition_params,
            landing_type = EXCLUDED.landing_type,
            landing_params = EXCLUDED.landing_params,
        ...
        console.error(`⚠️ ${rule.pushCode} 삽입 실패:`, error);
        skippedCount++;
      }
    }
