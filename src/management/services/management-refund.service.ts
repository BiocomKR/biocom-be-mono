...
export class ManagementRefundService {
  ...
  private readonly tossSecretKey = process.env.TOSS_PAYMENTS_SECRET_KEY;
  ...
  private async cancelTossPayment(
    ...
    const url = `${this.tossBaseUrl}/payments/${paymentKey}/cancel`;
    const headers = {
    ...
    if (!response.ok) {
      ...
      this.logger.error(`토스페이먼츠 취소 실패: ${paymentKey}`, errorData);
      throw new Error(errorData.message || '결제 취소 실패');
    }

    const data = await response.json();
    this.logger.log(`토스페이먼츠 취소 성공: ${paymentKey}`);
    return data;
  }
  ...
  async completeReturn(orderNumber: string, returnId: number, returnTrackingNumber?: string) {
    return await this.prisma.$transaction(async (tx) => {
      ...
      try {
        const tossResponse = await this.cancelTossPayment(
