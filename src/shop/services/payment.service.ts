...
export class PaymentService {
  ...
  async confirmPayment(userId: number, dto: ConfirmPaymentDto): Promise<PaymentResponseDto> {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      // 챌린지 상품 티켓 발급 (동시성 방어: seq 기반 deterministic 생성 + DB unique 제약)
        ...
        if (item.product.categoryCode === 'CHALLENGE') {
          ...
          for (let seq = 1; seq <= item.quantity; seq++) {
            try {
              ...
              if (error.code === 'P2002') {
                this.logger.log(`챌린지 티켓 이미 존재: orderItemId=${item.id}, seq=${seq}`);
                continue;
              }
              throw error; // 그 외 에러는 재throw
            }
          }
          this.logger.log(`챌린지 티켓 발급 완료: orderItemId=${item.id}, quantity=${item.quantity}`);
        }
      }
