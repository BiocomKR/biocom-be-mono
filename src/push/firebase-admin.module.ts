      ...
      useFactory: (configService: ConfigService): FirebaseApps => {
        ...
        try {
          ...
          if (fs.existsSync(devFullPath)) {
            ...
            console.log('✅ Firebase Dev 앱 초기화 완료');
          } else {
            console.warn('⚠️  Firebase Dev 키 파일 없음:', devFullPath);
          }
        } catch (error) {
          console.error('❌ Firebase Dev 앱 초기화 실패:', error);
        }
        ...
        try {
          ...
          if (fs.existsSync(prodFullPath)) {
            ...
            console.log('✅ Firebase Prod 앱 초기화 완료');
          } else {
            console.warn('⚠️  Firebase Prod 키 파일 없음:', prodFullPath);
          }
        } catch (error) {
          console.error('❌ Firebase Prod 앱 초기화 실패:', error);
        }
