...
async function migrateBalanceGameSteps(excelPath) {
  console.log('🎮 [운영 DB] 밸런스게임 STEPS 마이그레이션 시작...\n');
  ...
  try {
    ...
    // 시퀀스 초기화
    await prisma.$executeRawUnsafe("SELECT setval('balance_game_steps_id_seq', 1, false);");
    ...
    await prisma.$executeRawUnsafe(`SELECT setval('balance_game_steps_id_seq', ${nextVal}, false);`);
