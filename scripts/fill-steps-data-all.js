...
/**
 * STEPS 시트의 step_number와 parent_row_id 자동 채우기 (전체 버전)
 *
 * 올바른 규칙:
...
async function fillStepsDataAll() {
  console.log('🚀 STEPS 데이터 전체 재작업 시작 (2번째 라인부터)...\n');
  ...
  console.log('🔄 2번째 라인부터 전체 채우기 시작...\n');
  ...
  for (let i = 0; i < allRows.length; i++) {
    ...
    if (currentRow.step_type === 'QUESTION') {
      ...
      if (prevRow && prevRow.step_type === 'QUESTION') {
        ...
        newStepNumber = (prevRow.newStepNumber || 1) + 1;
  ...
  console.log(`\n✅ 데이터 채우기 완료!`);
  console.log(`  - 채운 행: ${fillCount}개 (2번째 줄부터)`);
  console.log(`  - 스킵한 행: ${skipCount}개\n`);

  // 파일 저장
  const outputPath = '/Users/daegilchoi/Downloads/balance-game-template-v2_완료_수정완료.xlsx';
  await workbook.xlsx.writeFile(outputPath);
  console.log(`💾 파일 저장 완료: ${outputPath}\n`);

  // 샘플 검증 (2~30번 라인)
  console.log('🔍 샘플 검증 (2~30번 라인):\n');
  const sampleRows = allRows.filter(r => r.rowNumber >= 2 && r.rowNumber <= 30);
